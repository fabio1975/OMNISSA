...
param(
        ...
        [string]$Verbose, [string]$LogPath = "C:\Attimo\UAGDeploy_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
     )
...
#Invoke-RestMethod -Method Get -Uri "https://$DstServerPort/rest/v1/config/settings?format=INI" -Headers @{Authorization="Bearer $token"} -SkipCertificateCheck -OutFile $PathINI
Invoke-RestSafe -Method GET -Uri "https://$DstServerPort/rest/v1/config/settings?format=INI" -Headers @{Authorization="Bearer $token"} -OutFile $PathINI
